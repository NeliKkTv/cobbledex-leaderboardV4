...
class LeaderboardImageGenerator:
    ...
    def _draw_leaderboard_section_modern(self, img: Image.Image, draw: ImageDraw.Draw,
                                        ...
                                        nb_players: int):
        """Draw a modern Minecraft-style leaderboard section."""
        padding = 20
        ...
        # Draw title
        font_title = ImageFont.truetype("fonts/Minecraft-Seven_v2.ttf", 28)
        ...
        entry_height = 70
        font_name = ImageFont.truetype("fonts/minecraft.ttf", 20)
        font_score = ImageFont.truetype("fonts/Minecraft-Seven_v2.ttf", 18)
        
        for rank, (_, player) in enumerate(df.iloc[:nb_players].iterrows(), 1):
            entry_y = entry_start_y + (rank - 1) * entry_height
            entry_x = panel_x + 15
    ...
    def generate_top_image(self, leaderboards: List[pd.DataFrame], 
        ...
        """Generate modern Minecraft-style leaderboards image and upload to GitHub.
        
        Returns:
            GitHub raw URL if upload successful, None otherwise
        """
        # Get configuration
        ldb_width = self.config.get_int("TOPIMAGE", "Width")
        ldb_height = self.config.get_int("TOPIMAGE", "Height")
        nb_players = self.config.get_int("TOPIMAGE", "NbPlayers")
        image_path = self.config.get("TOPIMAGE", "ImagePath")
        
        # Calculate dimensions
        entry_height = 70
        base_width = 650
        base_height = 200 + (nb_players * entry_height)
        width = base_width * ldb_width
        height = base_height * ldb_height
        
        # Create gradient background (dark blue to black, Minecraft style)
        img = self._create_gradient_background(
            width, height,
            (15, 15, 35, 255),   # Dark blue-ish
            (5, 5, 15, 255)       # Almost black
        )
        ...
        # Draw each leaderboard
        for i, (df, title) in enumerate(zip(leaderboards, titles)):
            x_offset = (i % ldb_width) * base_width
            y_offset = (i // ldb_width) * base_height
            
            img = self._draw_leaderboard_section_modern(
                img, ImageDraw.Draw(img, 'RGBA'), df,
                x_offset, y_offset, title,
